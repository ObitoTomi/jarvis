...
while running:
    ...
    wave_strength = (amplitude / 100000)  # Ajuster l'intensité des vagues
    ...
    for i in range(num_points):
        ...
        noise = wave_strength * math.sin(8 * angle + pygame.time.get_ticks() * 0.001)
        ...
        noise = wave_strength * math.sin(12 * angle + pygame.time.get_ticks() * 0.001)
        ...
        # Cercle 3
        angle = (i / num_points) * 2 * math.pi
        noise = wave_strength * math.sin(6 * angle - pygame.time.get_ticks() * 0.001) 
        radius = base_radius + noise
        x = WIDTH // 2 + int(radius * math.cos(angle))
        y = HEIGHT // 2 + int(radius * math.sin(angle))
        points.append((x, y))
